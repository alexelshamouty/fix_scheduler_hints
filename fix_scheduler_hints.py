...
credentials = json.loads(subprocess.check_output(["vault","read","-format","json","secret/mysql/nova"]))
...
def fix(conn, row):
  ...
  with conn.cursor() as cur:
    cur.execute("select uuid from instance_groups inner join instance_group_member on instance_groups.id=instance_group_member.group_id where instance_group_member.instance_uuid= %s;",instance_uuid)
    ...
    if server_group:
      ...
      print(information)
      cur.execute("update request_specs set spec = %s where instance_uuid = %s;",
...
def fix_instance(instance):
  with pymysql.connect(host=MYSQL_HOST, user=credentials["data"]["user"]
      , password=credentials["data"]["password"], db="novaapi") as conn:
    with conn.cursor() as cur:
